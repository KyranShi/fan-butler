# diag11.ps1 - ASCII only. Test duty writes in STANDARD mode (with duty readback verification).
...
"=== 1. set STANDARD, settle 12s ==="
[void](Raw 0x30 0x45 @([byte]0x01,[byte]0x00))
Start-Sleep -Seconds 12
"mode=$(ModeGet) fanAvg=$(FanAvg) duty_z0=$(GetDuty 0x00) duty_z1=$(GetDuty 0x01)"
"=== 2. duty z0=60% in Standard ==="
[void](Raw 0x30 0x70 0x66 0x01 @([byte]0x00,[byte]0x3C))
Start-Sleep -Seconds 6
"fanAvg=$(FanAvg) duty_z0=$(GetDuty 0x00)  (expect 60, fans ~8-9)"
"=== 3. duty z0=30% ==="
[void](Raw 0x30 0x70 0x66 0x01 @([byte]0x00,[byte]0x1E))
Start-Sleep -Seconds 6
"fanAvg=$(FanAvg) duty_z0=$(GetDuty 0x00)  (expect 30, fans ~4-5)"
"=== 4. swapped-byte test z0: [66 01 3C 00] ==="
[void](Raw 0x30 0x70 0x66 0x01 @([byte]0x3C,[byte]0x00))
Start-Sleep -Seconds 6
"fanAvg=$(FanAvg) duty_z0=$(GetDuty 0x00) duty_z1=$(GetDuty 0x01)"
"=== 5. persistence 30s ==="
foreach($i in 1..3){
  Start-Sleep -Seconds 10
  "t+$($i*10)s fanAvg=$(FanAvg) duty_z0=$(GetDuty 0x00) mode=$(ModeGet)"
}
"=== 6. back to OPTIMAL ==="
